...
SKKState::SKKState(SKKInputEnvironment* env, SKKInputEngine* editor)
    : context_(env->InputContext()),
      ...
      configuration_(env->InputSessionParameter()->Config()), editor_(editor),
      completer_(editor_), selector_(editor_, window_) {}

SKKState::SKKState(const SKKState& src)
    : context_(src.context_), messenger_(src.messenger_), window_(src.window_),
      configuration_(src.configuration_), editor_(src.editor_),
      completer_(editor_), selector_(editor_, window_) {}

void SKKState::ToString(
    const Handler handler, const Event& event, std::string& result
) {
    static const char* systemEvent[] = {
        "PROBE", "<<ENTRY>>", "<<INIT>>", "<<EXIT>>"};
