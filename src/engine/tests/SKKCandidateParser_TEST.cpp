...
int main() {
    ...
    parser.Parse("/候補1/");
    assert(
        parser.Candidates().size() == 1 &&
        parser.Candidates()[0] == SKKCandidate("候補1")
    );

    parser.Parse("/候補1/候補2;アノテーション/候補3/");
    assert(
        parser.Candidates().size() == 3 &&
        parser.Candidates()[1] == SKKCandidate("候補2")
    );
    ...
    assert(
        parser.Candidates().size() == 1 && parser.Hints().size() == 1 &&
        parser.Hints()[0] == hint
    );
