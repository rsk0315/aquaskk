...
int main() {
    ...
    assert(
        keymap.Fetch('j', 0, SKKKeyState::CTRL) == SKKEvent(SKK_JMODE, 'j', 0)
    );
    assert(keymap.Fetch(0x03, 0, 0) == SKKEvent(SKK_ENTER, 0x03, 0));
    assert(keymap.Fetch(0x09, 0, 0) == SKKEvent(SKK_TAB, 0x09, 0));
    assert(
        keymap.Fetch('i', 0, SKKKeyState::CTRL) == SKKEvent(SKK_TAB, 'i', 0)
    );
    assert(
        keymap.Fetch('g', 0, SKKKeyState::CTRL) == SKKEvent(SKK_CANCEL, 'g', 0)
    );
