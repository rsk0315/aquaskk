...
class DebugLoader : public SKKDictionaryLoader {
    ...
    virtual bool NeedsUpdate() { return state_; }
...
public:
    void Clear() { notified_ = false; }

    SKKDictionaryFile& File() { return file_; }
...
void test(const char* msg, TestData* suite) {
    ...
    for (int index = 0; suite[index].comment != 0; ++index) {
        ...
        if (observer.IsNotified() != data.notified) {
            std::cerr << "notify test failed: " << msg << data.comment
                      << std::endl;
        }

        if (observer.File().IsEmpty() != data.empty) {
            std::cerr << "empty test failed: " << msg << data.comment
                      << std::endl;
        }
    }
}
