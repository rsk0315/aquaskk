...
int main() {
    ...
    assert(
        backend.Find(SKKEntry("よi", "い"), suite) &&
        suite.ToString() == "/良/好/酔/善/"
    );
    assert(!backend.Find(SKKEntry("NOT-EXIST"), suite));
    assert(
        backend.Find(SKKEntry("たんごとうろく"), suite) &&
        suite.ToString() == "/単語登録/"
    );
    ...
    // 補完
    assert(
        backend.Complete("か", result) && result[0] == "かなめ" &&
        result[1] == "かんじ"
    );

    // 検索
    assert(
        backend.Find(SKKEntry("あr", "り"), suite) &&
        suite.ToString() == "/有/[り/有/]/"
    );
    assert(
        backend.Find(SKKEntry("かなめ"), suite) && suite.ToString() == "/要/"
    );
    ...
    assert(
        backend.Find(SKKEntry("おおk", "き"), suite) &&
        suite.ToString() == "/大/"
    );
    assert(
        backend.Find(SKKEntry("むし"), suite) && suite.ToString() == "/蒸し/虫/"
    );
}
