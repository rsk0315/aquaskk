...
class MockFrontEnd : public SKKFrontEnd, public SKKInputModeListener {
    ...
    virtual void InsertString(const std::string& str) { result_.fixed += str; }
    ...
    virtual void
    ComposeString(const std::string& str, int convertFrom, int convertTo) {
        result_.marked = str;
        result_.pos = 0;
    }

    virtual std::string SelectedString() { return selected_string_; }

    virtual void SelectInputMode(SKKInputMode mode) { result_.mode = mode; }
