...
const std::string& SKKHttpDictionaryLoader::FilePath() const { return path_; }
...
bool SKKHttpDictionaryLoader::request(net::socket::tcpstream& http) {
    ...
    strftime(
        timestamp, sizeof(timestamp), "%a, %d %b %Y %T GMT",
        gmtime(&st.st_mtime)
    );
...
bool SKKHttpDictionaryLoader::download(
    net::socket::tcpstream& http, int length
) {
    if (!length)
        return false;
    ...
    if (new_size != length) {
        std::cerr
            << "SKKHttpDictionaryLoader::download(): size conflict: expected="
    ...
    if (rename(tmp_path_.c_str(), path_.c_str()) != 0) {
        std::cerr
            << "SKKHttpDictionaryLoader::download(): rename failed: errno="
            << errno << std::endl;
        return false;
    }
