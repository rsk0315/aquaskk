...
// 文字列一括置換ユーティリティ
void translate(
    std::string& src, const std::string& from, const std::string& to
) {
...
SKKKeymapEntry::SKKKeymapEntry(
    ...
    if (index < 0) {
        std::cout << "SKKKeymapEntry::SKKKeymapEntry(): invalid key name["
                  << configKey << "]" << std::endl;
        return;
    }
...
bool SKKKeymapEntry::IsEvent() const { return type_ == TYPE_EVENT; }

bool SKKKeymapEntry::IsAttribute() const { return type_ == TYPE_ATTRIBUTE; }

bool SKKKeymapEntry::IsNot() const { return not_; }

int SKKKeymapEntry::Symbol() const { return symbol_; }
...
std::string SKKKeymapEntry::setup(const std::string& str) {
    ...
    while (buf >> tmp) {
        ...
        if (tmp == "hex")
            label_ |= LABEL_HEX;
        if (tmp == "keycode")
            label_ |= LABEL_KEYCODE;

        if (tmp == "shift")
            mods_ |= SKKKeyState::SHIFT;
        if (tmp == "ctrl")
            mods_ |= SKKKeyState::CTRL;
        if (tmp == "alt")
            mods_ |= SKKKeyState::ALT;
        if (tmp == "meta")
            mods_ |= SKKKeyState::META;
    }
