...
class SKKCandidateSuite {
    ...
    public:
        ...
        bool operator()(const SKKCandidate& item) const {
            return !item.IsEmpty() &&
                   std::find(container_.begin(), container_.end(), item) !=
                       container_.end();
        }
    };

    void
    add(SKKCandidateContainer& container, const SKKCandidateContainer& src) {
        std::remove_copy_if(
            src.begin(), src.end(), std::back_inserter(container),
            contains(container)
        );
    }

    void
    update(SKKCandidateContainer& container, const SKKCandidate& candidate) {
        remove(container, candidate);
        container.push_front(candidate);
    }

    void
    remove(SKKCandidateContainer& container, const SKKCandidate& candidate) {
        container.erase(
            std::remove(container.begin(), container.end(), candidate),
            container.end()
        );
    }
    ...
    void remove_hint(const SKKCandidate& candidate) {
        ...
        for (SKKOkuriHintIterator iter = hints_.begin(); iter != hints_.end();
             ++iter) {
    ...
    SKKOkuriHintIterator find_okuri(const std::string& okuri) {
        return std::find_if(
            hints_.begin(), hints_.end(), CompareOkuriHint(okuri)
        );
    }

    const std::string flatten(
        const SKKCandidateContainer& container, bool exclude_avoid_study
    ) const {
        std::string result;
...
public:
    ...
    SKKCandidateSuite(const std::string& line) { Parse(line); }
    ...
    bool IsEmpty() const { return candidates_.empty(); }
    ...
    template <typename Predicate> void RemoveIf(const Predicate& pred) {
        if (!hints_.empty()) {
            ...
            // pred が true の要素を remove して copy するので、not1
            // で反転させる
            std::remove_copy_if(
            ...
            for (unsigned index = 0; !hints_.empty() && index < removed.size();
                 ++index) {
        ...
        candidates_.erase(
            std::remove_if(candidates_.begin(), candidates_.end(), pred),
            candidates_.end()
        );
    }
    ...
    SKKCandidateContainer& Candidates() { return candidates_; }

    SKKOkuriHintContainer& Hints() { return hints_; }
